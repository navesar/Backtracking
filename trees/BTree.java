...
public class BTree<T extends Comparable<T>> {
    ...
    protected Deque<Node<T>> cn = new ArrayDeque<Node<T>>();
    protected Deque<Integer> vf = new ArrayDeque<Integer>();
	...
	public void insert(T value) {
		Node<T> stop = new Node<T>(null,4);
		Node<T> end = new Node<T>(stop,4);
		Node<T> cnt = new Node<T>(null,4);
		cn.addLast(end);
		boolean splitted=false;
		vf.addLast((Integer)value);
		if (root == null) {
